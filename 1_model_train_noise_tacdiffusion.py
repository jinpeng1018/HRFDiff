...
SAVE_DATA_DIR = "output/tacdiffusion_20251211" 
os.makedirs(SAVE_DATA_DIR, exist_ok=True)

#LOG_DIR = "logs/fit/mohusuanzi" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S_noise_mohusuanzi")
LOG_DIR = "logs/fit/tacdiffusion_20251211" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S_1")
...
Model_save_name = "tacdiffusion_20251211.pth"
...
def apply_noise_augmentation(x_batch, noise_prob=0.3, noise_magnitude=0.1):
    ...
    noise_dims = list(range(0, 12)) + list(range(18, 30))  # 1~12 和 19~30（含）
...
for ep in tqdm(range(n_epoch), desc="Epoch"):
    ...
    for x_batch, y_batch in pbar:
        ...
        if curriculum in ["blur", "downsample"]:
            ...
            # 对需要处理的部分应用模糊操作 
            if curriculum == "blur":
                part1 = gaussian_1d_smoothing(part1, scale)
                part3 = gaussian_1d_smoothing(part3, scale)
            elif curriculum == "downsample":
                part1 = downsample_1d(part1, scale)
                part3 = downsample_1d(part3, scale)
            # 重新组合所有部分
            x_batch = torch.cat([part1, part2, part3, part4], dim=1)
        # 计算损失
    ...
    if (ep + 1) % 300 == 0:  # Validate every 5 epochs
        model.eval()
